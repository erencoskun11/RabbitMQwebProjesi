...
namespace FileCreateWorkerService
{
    public class Worker : BackgroundService
    {
        ...
        private async Task Consumer_Received(object sender, BasicDeliverEventArgs @event)
        {
            ...
            MultipartFormDataContent multipartFormDataContext = new();

            multipartFormDataContext.Add(new ByteArrayContent(ms.ToArray()), "file", Guid.NewGuid
                ().ToString() + ".xlsx");
            ...
            {
                var response = await httpClient.PostAsync($"{baseUrl}?fileId={createExcelMessage.FileId}",multipartFormDataContext);
